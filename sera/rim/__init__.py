"""Sera's peripheral agent layer -- agents on the RIM, deterministic instrument at the hub.

Named `rim/` (not `agents/`) for two reasons. It cannot be confused with the existing singular
`agent/` package, which is the Sera SDK loop. And it puts the architectural claim in
the import path: `from sera.rim.ingest import ...` tells you at the call site that this
