...
Run:  python -m sera.serve      (see serve.py)
"""
...
# app.py lives at sera/api/app.py; the served frontend is bundled inside
# the package at sera/frontend/ — two dirname() hops (api -> sera).
...
# Sera's concordance table (the 2x2(+1) verdicts), loaded once from the precomputed
# parquet artifact. Sera is served alongside Sera from the same process; this
# cache stays empty (and its routes 503) if the artifact hasn't been built — the
# deterministic Sera paths never depend on it.
...
# The ground-truth panel: canonical IL-2 regulators recovered by Sera + the aggregate Spearman.
_GROUND_TRUTH: dict = {}
...
app = FastAPI(title="Sera", version="0.1.0", lifespan=_lifespan)
...
# ---- Sera: cross-modality concordance verdicts (served alongside Sera) -----
...
_ANSWER_SYSTEM = (
    "You are the assistant inside Sera, a gene-evidence reconciliation tool for immunology "
    ...
    "What Sera does: for a candidate gene it compares TWO independent CRISPR screens measuring "
...
def ground_truth() -> dict:
    """The ground-truth panel: how Sera recovers canonical IL-2 regulators + the aggregate
...
async def chat(body: dict) -> StreamingResponse:
    ...
    # The chat endpoint runs the Sera mRNA×protein reconciliation agent (its own tools +
    # prompt). The former Sera shortlist agent has been removed; `run_triage` is the
    # agent-agnostic driver, so building Sera's options here is the only agent it serves.
    from ..llm.sera_prompt import build_sera_options
    agent_options = build_sera_options()
...
# Sera is the default landing page. An explicit "/" route is matched BEFORE the catch-all
# StaticFiles mount below, so the bare root redirects to the Sera app instead of letting the
# mount serve a directory listing. Every /api/* route and /sera-app.html is unchanged.
@app.get("/", include_in_schema=False)
def root() -> RedirectResponse:
    return RedirectResponse(url="/sera-app.html")
