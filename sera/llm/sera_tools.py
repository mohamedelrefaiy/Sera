"""Sera agent tools: expose the mRNA×protein reconciliation to Claude via the Agent SDK.

The Sera chat is a REAL reasoning agent, not a fixed router: it reads the message, decides
...
or overrides the verdict (the "agents on the rim" invariant, see docs/architecture/SERA_AGENT_BUILD.md).
...
@tool(
    "known_biology",
    "Answer whether Sera recovers KNOWN IL-2 biology — the validation question, corpus-wide (not "
...
async def known_biology(args):
    ...
    plain = {
        ...
        "takeaway": "the recovery of the protein-only regulators is the evidence Sera works",
    }
    return _view(
        {"plain": plain,
...
@tool(
    "hitlist_biology",
    "Answer what BIOLOGY the replicated hits share — corpus-wide, not one gene. Sera's replicated "
...
def build_sera_server():
    """The in-process MCP server hosting Sera's tools."""
...
SERA_ALLOWED_TOOLS = [
    "mcp__sera__reconcile_gene",
    "mcp__sera__compare_conditions",
    "mcp__sera__gene_evidence",
    "mcp__sera__known_biology",
    "mcp__sera__draft_decision_brief",
    "mcp__sera__hitlist_biology",
