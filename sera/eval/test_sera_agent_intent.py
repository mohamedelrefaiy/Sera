"""SERA AGENT-INTENT GATE — the agent's tools must let the question choose the condition.
...
House discipline (mirrors test_sera_sanity / test_tools):
...
Init `sera.core` before `sera.llm` to sidestep the pre-existing llm<->core
...
Run:  pytest eval/test_sera_agent_intent.py
"""
...
import sera.core  # noqa: E402,F401  (initialize core before llm — see module docstring)
from sera.llm import sera_prompt, sera_tools  # noqa: E402
...
def test_reconcile_defaults_to_stim48hr_when_no_condition_given():
    ...
    vu = _view(_run(sera_tools.reconcile_gene.handler({"gene": _GENE})))
...
def test_reconcile_honours_an_explicit_condition():
    ...
    out = _run(sera_tools.reconcile_gene.handler({"gene": _GENE, "condition": "Rest"}))
...
def test_reconcile_condition_is_case_and_alias_insensitive():
    ...
    for raw, want in [("rest", "Rest"), ("resting", "Rest"), ("48h", "Stim48hr"),
        ...
        vu = _view(_run(sera_tools.reconcile_gene.handler({"gene": _GENE, "condition": raw})))
...
def test_reconcile_unknown_condition_falls_back_not_crashes():
    ...
    vu = _view(_run(sera_tools.reconcile_gene.handler({"gene": _GENE, "condition": "nonsense"})))
...
def test_compare_conditions_tool_exists_and_is_allowed():
    assert hasattr(sera_tools, "compare_conditions"), "compare_conditions tool is missing"
    assert "mcp__sera__compare_conditions" in sera_tools.SERA_ALLOWED_TOOLS
    server_tools = {t.name for t in (
        sera_tools.reconcile_gene, sera_tools.gene_evidence,
        sera_tools.known_biology, sera_tools.compare_conditions)}
...
def test_compare_all_conditions_returns_each_code_verdict_in_order():
    ...
    out = _run(sera_tools.compare_conditions.handler({"gene": _GENE}))
    payload, vu = _payload(out), _view(out)
...
def test_compare_respects_a_requested_condition_subset():
    ...
    out = _run(sera_tools.compare_conditions.handler(
...
def test_compare_unknown_gene_is_honest_not_fabricated():
    out = _run(sera_tools.compare_conditions.handler({"gene": "NOTAGENE"}))
    payload = _payload(out)
...
def test_compare_payload_is_words_only_no_leaked_statistics():
    ...
    payload = _payload(_run(sera_tools.compare_conditions.handler({"gene": _GENE})))
...
def test_prompt_routes_time_course_questions_to_compare():
    ...
    p = sera_prompt.SERA_SYSTEM_PROMPT.lower()
...
def test_draft_decision_brief_emits_a_plan_view_with_the_whole_brief():
    ...
    out = _run(sera_tools.draft_decision_brief.handler({"gene": _GENE}))
    vu = _view(out)
...
def test_draft_decision_brief_never_fabricates_for_an_unknown_gene():
    out = _run(sera_tools.draft_decision_brief.handler({"gene": "NOTAGENE12345"}))
    payload = _payload(out)
...
def test_draft_decision_brief_is_registered_and_prompt_routes_to_it():
    assert "mcp__sera__draft_decision_brief" in sera_tools.SERA_ALLOWED_TOOLS
    p = sera_prompt.SERA_SYSTEM_PROMPT.lower()
...
# The dogfooding gap: a scientist asking "find new targets" was told Sera can't, even though
...
def test_rank_targets_returns_code_ranked_closed_set():
    out = _run(sera_tools.rank_targets.handler({"limit": 6}))
    payload, vu = _payload(out), _view(out)
    ...
    for c in cands:
        assert c["gene"] in sera_tools._GENES, f"{c['gene']} is not a real screen gene"
...
def test_rank_targets_leaks_no_raw_statistic_to_the_agent():
    ...
    out = _run(sera_tools.rank_targets.handler({}))
    payload = _payload(out)
...
def test_rank_targets_limit_is_clamped_not_crashed():
    ...
    assert len(_view(_run(sera_tools.rank_targets.handler({"limit": 9999})))["candidates"]) \
        <= sera_tools._RANK_MAX_N
    assert len(_view(_run(sera_tools.rank_targets.handler({"limit": 0})))["candidates"]) >= 1


def test_rank_targets_is_registered_and_prompt_routes_discovery_to_it():
    assert "mcp__sera__rank_targets" in sera_tools.SERA_ALLOWED_TOOLS
    assert sera_tools.rank_targets.name == "rank_targets"
    p = sera_prompt.SERA_SYSTEM_PROMPT.lower()
    ...
    # the prompt must NOT still claim Sera can't find targets
    assert "sera can" in p and "front door" in p, \
        "prompt should affirm Sera CAN surface candidates"
...
def test_sketch_of_a_curated_gene_emits_the_pathway_map_figure():
    ...
    vu = _view(_run(sera_tools.sketch_gene.handler({"gene": "ITK"})))
...
def test_sketch_of_a_non_curated_gene_stays_the_bench_cartoon():
    ...
    vu = _view(_run(sera_tools.sketch_gene.handler({"gene": _GENE})))
...
def test_sketch_routing_honours_the_requested_condition():
    ...
    out = _run(sera_tools.sketch_gene.handler({"gene": "ITK", "condition": "Stim8hr"}))
