...
Nothing in Sera is "done" until this passes: the join of the mRNA screen (Zhu) and the
...
Run as a gate:    python eval/test_sera_sanity.py     (prints PASS/FAIL + numbers)
Run under pytest: pytest eval/test_sera_sanity.py
...
from sera.core.concordance import Config, load_protein  # noqa: E402
...
def test_direction_is_never_guessed_for_a_zero_effect():
    ...
    from sera.core.concordance import _direction
...
def test_classify_marks_an_unassayed_protein_side_untested():
    ...
    from sera.core.concordance import Config, MrnaEffect, classify
    cfg = Config.load()
...
def _run_gate() -> bool:
    ...
    print(f"\n{'PASS' if ok else 'FAIL'} — Sera sanity gate")
    return ok
