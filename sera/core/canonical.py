...
Sera's thesis is *agents on the rim, deterministic instrument at the hub*. This module is
...
the *Sera* ranking instrument: it reads one screen's columns into a GeneRecord to rank
genes WITHIN that screen. It has no cytokine, modality, significance-regime or sign axis --
because ranking inside one screen holds all of those constant. Sera reconciles ACROSS
screens, where exactly those things silently differ, so it needs axes for them. (Concretely:
ScreenSchema reads Schmidt's `phenotype` column as one opaque condition, "CD4+ IL2"; Sera
