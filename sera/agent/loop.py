"""The agent loop — Claude orchestrates the Sera tools.
...
`options` (see llm/sera_prompt.build_sera_options) so the model surface is
...
async def run_triage(task: str, on_message=None, options=None) -> list:
    ...
    `options` (a ClaudeAgentOptions) selects WHICH agent runs — e.g. the Sera
    reconciliation agent from llm/sera_prompt.build_sera_options(). The driver is
    ...
    if options is None:
        raise ValueError(
            ...
            "agent's prompt + tools, e.g. build_sera_options()."
        )
    messages: list = []
