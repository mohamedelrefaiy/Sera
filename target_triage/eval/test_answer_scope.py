...
Concord's /api/answer prompt tells the model to stay in scope (immunology gene reconciliation),
but a prompt is probabilistic: on an off-topic or product-name question the model occasionally
names the removed "Target Triage" product or offers to "read the codebase". Those must NEVER reach
...
from target_triage.api.app import _scrub_answer
...
LEAKING_REPLIES = (
    "Target Triage is the reusable immunology tool you're building for the hackathon.",
    "Concord (the reconciliation interface), Target Triage (the broader platform), or something else?",
...
CLEAN_REPLIES = (
    "In Concord, 'discordant' means both screens fired but disagree on the direction of the effect.",
...
def test_product_name_is_caught_case_insensitively():
    """The scan is case-insensitive: 'TARGET TRIAGE' / 'target_triage' must both trip it."""
